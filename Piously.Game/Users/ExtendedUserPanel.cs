...
namespace Piously.Game.Users
{
    public abstract class ExtendedUserPanel : UserPanel
    {
        ...
        private PiouslySpriteText statusMessage;
        ...
        private void load()
        {
            BorderColour = ColorProvider?.Light1 ?? Colors.GreyVioletLighter;
        ...
        protected FillFlowContainer CreateStatusMessage(bool rightAlignedChildren)
        {
            ...
            statusContainer.Add(LastVisitMessage = new TextFlowContainer(t => t.Font = PiouslyFont.GetFont(size: 12, weight: FontWeight.SemiBold)).With(text =>
            {
            ...
            statusContainer.Add(statusMessage = new PiouslySpriteText
            {
                Anchor = alignment,
                Origin = alignment,
                Font = PiouslyFont.GetFont(size: 14, weight: FontWeight.SemiBold)
            });
        ...
        private void displayStatus(UserStatus status, UserActivity activity = null)
        {
            if (status != null)
            {
                ...
                if (activity != null && !(status is UserStatusOffline))
                {
                    ...
                    statusIcon.FadeColour(activity.GetAppropriateColour(Colors), 500, Easing.OutQuint);
                    return;
                }

                // Otherwise use only status
                statusMessage.Text = status.Message;
                statusIcon.FadeColour(status.GetAppropriateColor(Colors), 500, Easing.OutQuint);
