...
namespace Piously.Game.Online.API
{
    ...
    {
        private readonly PiouslyConfigManager config;

        private readonly OAuth authentication;

        //NEED TO SETUP OAUTHENTICATION WITH SERVER AT SOME POINT
        public string Endpoint => @"https://piously.xyz"; //Domain TBD
        private const string client_id = @"5"; //temp
        private const string client_secret = @"9FCD90E527541F4D57196921C0841DAB"; //temp
        ...
        public APIAccess(PiouslyConfigManager config)
        {
            this.config = config;
            ...
            ProvidedUsername = config.Get<string>(PiouslySetting.Username);

            authentication.TokenString = config.Get<string>(PiouslySetting.Token);
        ...
        private void onTokenChanged(ValueChangedEvent<OAuthToken> e) => config.Set(PiouslySetting.Token, config.Get<bool>(PiouslySetting.SavePassword) ? authentication.TokenString : string.Empty);
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                switch (State.Value)
                {
                    ...
                    case APIState.Connecting:
                        ...
                        config.Set(PiouslySetting.Username, config.Get<bool>(PiouslySetting.SaveUsername) ? ProvidedUsername : string.Empty);
