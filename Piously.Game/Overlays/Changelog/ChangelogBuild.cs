...
namespace Piously.Game.Overlays.Changelog
{
    public class ChangelogBuild : FillFlowContainer
    {
        ...
        private void load(PiouslyColor colours, OverlayColorProvider colourProvider)
        {
            foreach (var categoryEntries in Build.ChangelogEntries.GroupBy(b => b.Category).OrderBy(c => c.Key))
            {
                ChangelogEntries.Add(new PiouslySpriteText
                {
                    Text = categoryEntries.Key,
                    Font = PiouslyFont.GetFont(weight: FontWeight.Bold, size: 18),
                    Margin = new MarginPadding { Top = 35, Bottom = 15 },
                });

                var fontLarge = PiouslyFont.GetFont(size: 16);
                var fontMedium = PiouslyFont.GetFont(size: 12);
                    ...
                    Child = new FillFlowContainer<SpriteText>
                    {
                        ...
                        Children = new[]
                        {
                            new PiouslySpriteText
                            {
                                ...
                                Font = PiouslyFont.GetFont(weight: FontWeight.Medium, size: 19),
                            },
                            new PiouslySpriteText
                            {
                                Text = " ",
                            ...
                            new PiouslySpriteText
                            {
                                ...
                                Font = PiouslyFont.GetFont(weight: FontWeight.Light, size: 19),
