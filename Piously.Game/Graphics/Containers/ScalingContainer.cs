...
namespace Piously.Game.Graphics.Containers
{
    ...
    public class ScalingContainer : Container
    {
        ...
        private class ScalingDrawSizePreservingFillContainer : DrawSizePreservingFillContainer
        {
            ...
            private void load(PiouslyConfigManager osuConfig)
            {
                if (applyUIScale)
                {
                    uiScale = osuConfig.GetBindable<float>(PiouslySetting.UIScale);
        ...
        {
            scalingMode = config.GetBindable<ScalingMode>(PiouslySetting.Scaling);
            scalingMode.ValueChanged += _ => updateSize();

            sizeX = config.GetBindable<float>(PiouslySetting.ScalingSizeX);
            sizeX.ValueChanged += _ => updateSize();

            sizeY = config.GetBindable<float>(PiouslySetting.ScalingSizeY);
            sizeY.ValueChanged += _ => updateSize();

            posX = config.GetBindable<float>(PiouslySetting.ScalingPositionX);
            posX.ValueChanged += _ => updateSize();

            posY = config.GetBindable<float>(PiouslySetting.ScalingPositionY);
        ...
        private class ScalingBackgroundScreen : BackgroundScreen
        {
