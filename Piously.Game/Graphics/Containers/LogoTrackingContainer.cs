...
{
    /// <summary>
    /// A container that handles tracking of an <see cref="PiouslyLogo"/> through different layout scenarios.
    /// </summary>
    public class LogoTrackingContainer : Container
    {
        public Facade LogoFacade => facade;

        protected PiouslyLogo Logo { get; private set; }
        ...
        public void StartTracking(PiouslyLogo logo, double duration = 0, Easing easing = Easing.None)
        {
            if (logo == null)
                throw new ArgumentNullException(nameof(logo));

            if (logo.IsTracking && Logo == null)
                throw new InvalidOperationException($"Cannot track an instance of {typeof(PiouslyLogo)} to multiple {typeof(LogoTrackingContainer)}s");
